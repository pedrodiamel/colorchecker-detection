...
process(const std::string& pathImage)
{
	...
	// load image color
	image = cv::imread(pathImage, CV_LOAD_IMAGE_COLOR);
...
process(const cv::Mat & image, const std::string &pathOut)
{
	...
	cv::drawContours(im_contour, contours, -1, cv::Scalar(255), 2, CV_AA);
	...
	for (size_t i = 0; i < colorCharts.size(); i++)
	{
		...
		int thickness_box = 2;
		cv::line(image_box, ibox[0], ibox[1], color_box, thickness_box, CV_AA);
		cv::line(image_box, ibox[1], ibox[2], color_box, thickness_box, CV_AA);
		cv::line(image_box, ibox[2], ibox[3], color_box, thickness_box, CV_AA);
		cv::line(image_box, ibox[3], ibox[0], color_box, thickness_box, CV_AA);
	...
	{
		mcc::CCheckerDraw cdraw(&(checkers[ck]));
	...
	) const
{
	...
	cv::cvtColor(bgrOut, grayOut, CV_BGR2GRAY);

	// PDiamel: wiener adaptative methods to minimize the noise effets 
	// by illumination

	mcc::CWienerFilter filter;
...
findCandidates(
	...
	std::vector< mcc::CChart >& detectedCharts,
	int minContourLengthAllowed
	)
{
	std::vector<cv::Point>  approxCurve;
	std::vector<mcc::CChart>   possibleCharts;
	...
	{
		const mcc::CChart& m1 = possibleCharts[i];
		...
		{
			const mcc::CChart& m2 = possibleCharts[j];
...
void mcc::CCheckerDetector::
clustersAnalysis(
	const std::vector<mcc::CChart>& detectedCharts,
	...
	std::vector< std::vector<cv::Point2f> > &colorChartsOut)
{
	...
	for (size_t g = 0; g < Nc; g++)
	{
		...
		std::vector<mcc::CChart> chartSub;
		...
		mcc::CChartClassicModel::SUBCCMModel scm;
		scm.centers = cte;
	...
	)
{
	...
	for (size_t i = 0; i < n ; i++)
	{
		...
		// result
		checker.target = mcc::CChecker::MCC24;
		checker.N = 24;
		checker.box = ibox;
...
cost_function(const cv::Mat & img, const std::vector<cv::Point2f>& ibox)
{
	...
	//cv::cvtColor(im_rgb, im_lab, CV_BGR2Lab);
	cv::cvtColor(im_rgb, im_lab, CV_BGR2RGB);
