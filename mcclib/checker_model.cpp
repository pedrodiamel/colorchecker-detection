#include "checker_model.h"
#include "checker_model.h"
#include "checker_model.h"
#include <locale>

const float mcc::CChartClassicModel::chart[24][9] = {
...
mcc::CChartClassicModel::CChartClassicModel()
{
...
match(const SUBCCMModel & subModel, int iTheta, float & error, int & ierror) 
{
	...
	// RGB color space
	//cv::cvtColor(rgb_est, lab_est, CV_BGR2RGB);
	
	// Lab color space
	//rgb_est *= 1/255;
	cv::cvtColor(rgb_est, lab_est, CV_BGR2Lab);
...
draw(cv::Mat & img)
{
	...
	for (size_t i = 0, k; i < N; i++)
	{
		...
		cv::line(img, bcht[0], bcht[1], m_color, m_thickness, CV_AA);
		cv::line(img, bcht[1], bcht[2], m_color, m_thickness, CV_AA);
		cv::line(img, bcht[2], bcht[3], m_color, m_thickness, CV_AA);
		cv::line(img, bcht[3], bcht[0], m_color, m_thickness, CV_AA);
