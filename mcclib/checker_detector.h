...
public:
	...
	* \param[out] checker
	*/
	void getBestColorChecker(mcc::CChecker &checker) { checker = m_checkers[0]; }
	
	/// getColorChecker
	/** \brief get list color checker
	* \param[out] checkers list checker color
	*/
	void getListColorChecker(std::vector< mcc::CChecker > &checkers) { checkers = m_checkers; }
...
protected: // methods pipeline
	...
	*/
	virtual void
	findCandidates(const ContoursVector &contours, std::vector< mcc::CChart >& detectedCharts, int minContourLengthAllowed	);
	...
	*/
	virtual void
	clustersAnalysis(const std::vector< mcc::CChart > &detectedCharts, std::vector<int> &groups );
	...
	*/
	...
	checkerAnalysis(const cv::Mat &img, const cv::Mat &img_org,
		...
		std::vector<mcc::CChecker>  &checkers, float asp = 1);



protected:

	std::vector<mcc::CChecker> m_checkers;
	int m_fact_size;
	int m_num_ch;
	float m_min_error;
...
private: // methods aux
		...
		)
	{
		...
		cv::cvtColor(im_bgr, im_ycbcr, CV_BGR2YCrCb);
